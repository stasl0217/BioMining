...
class PDF2text_helper:
    def __init__(self, pdf_dir, inter_dir, txt_dir):
        self.pdf_dir = pdf_dir
        self.inter_dir = inter_dir
        self.txt_dir = txt_dir

    def mining_pdf(self, filename):
        if filename[-4:] != '.pdf':
            return

        inter_name = filename[:-4] + '.txt'

        fp = open(join(self.pdf_dir, filename), 'rb')
        # 来创建一个pdf文档分析器
        parser = PDFParser(fp)
        # 创建一个PDF文档对象存储文档结构
            ...
            for page in PDFPage.create_pages(document):
                ...
                for x in layout:
                    if (isinstance(x, LTTextBoxHorizontal)):
                        output_path = join(self.inter_dir, inter_name)
                        with open(output_path, 'a') as f:
                            f.write(x.get_text().encode('utf-8') + '\n')  # save intermediate result
        fp.close()

        # clean text
        # read from intermediate result
        with open(join(self.inter_dir, inter_name), 'r') as f:
            str = f.read()
            ...
            with open(join(self.txt_dir, inter_name), 'w') as fout:
                for char in result:
                    fout.write(self.clean_char(char))  # get rid of invalid XML characters

    def invalid_xml_remove(self, c):
        """
    ...
    def clean_char(self, char):
        ...
        char = self.invalid_xml_remove(char)
        replacements = [
            (u'\u201c', '\"'),
            (u'\u201d', '\"'),
